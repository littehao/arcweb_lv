<template>
    <div class="m-home">
        <div class="m-home-banner" :style="style">
            <div class="m-banner-logo">
                <img class="logo animated fadeInUp" src="~assets/images/home/lv.png" alt="">
                <div class="title animated fadeInUp">Live Video</div>
                <div class="info animated fadeInUp">一站式综合博娱直播系统的开创者与定义者</div>
            </div>
            <div class="m-banner-img">
                <div class="left">
                    <img class="bannerimg banner1 animated fadeInLeft" src="~assets/images/home/sy_yuansu_1.png" alt="">
                    <img class="bannerimg banner2 animated fadeInLeft" src="~assets/images/home/sy_yuansu_2.png" alt="">
                    <img class="bannerimg banner3 animated fadeInLeft" src="~assets/images/home/sy_yuansu_3.png" alt="">
                </div>
                <div class="right">
                    <img class="bannerimg banner1 animated fadeInRight" src="~assets/images/home/sy_yuansu_4.png" alt="">
                    <img class="bannerimg banner2 animated fadeInRight" src="~assets/images/home/sy_yuansu_5.png" alt="">
                    <img class="bannerimg banner3 animated fadeInRight" src="~assets/images/home/sy_yuansu_6.png" alt="">
                    <img class="bannerimg banner4 animated fadeInRight" src="~assets/images/home/sy_yuansu_7.png" alt="">
                </div>
            </div>
            <div class="control-down" @click="controldown">
                <div class="down-page" style="display:block;">
                    <i class="icon-darr"></i>
                </div>
            </div>
        </div>
        <div class="m-home-part">
            <div class="section">
               <div class="pic revealOnScroll" data-animation="fadeInUp">
                    <img class="img1" src="~assets/images/home/sy_dn_1.png" alt="">
                    <img class="img2" src="~assets/images/home/sy_sj_1.png" alt="">
               </div>
               <div class="info revealOnScroll" data-animation="fadeInUp">
                   <h3 class="title">倾力打造一站式综合博娱直播系统</h3>
                   <div class="lv">Live Video</div>
                   <div class="intro">努力提升用户体验，改变了目前游戏类直播产品不能很好融合的行业痛点，让玩家一边看直播，一边下注成为可能，开奖信息实时公布，实时的下注倒计时提醒</div>
               </div>
            </div>
            <div class="section">
               <div class="pic" style="height:221px">
                  <img src="~assets/images/avatar/tx_1.png" class="avatar tx1 revealOnScroll" data-animation="fadeInUp" alt="">
                  <img src="~assets/images/avatar/tx_2.png" class="avatar tx2 revealOnScroll" data-animation="fadeInUp" alt="">
                  <img src="~assets/images/avatar/tx_3.png" class="avatar tx3 revealOnScroll" data-animation="fadeInUp" alt="">
                  <img src="~assets/images/avatar/tx_4.png" class="avatar tx4 revealOnScroll" data-animation="fadeInUp" alt="">
               </div>
               <div class="info">
                   <h3 class="title revealOnScroll" data-animation="fadeInUp">全球博娱游戏直播的开创者</h3>
                   <div class="lv revealOnScroll" data-animation="fadeInUp">Live Video</div>
                   <div class="intro revealOnScroll" data-animation="fadeInUp">专业的培训让主播熟悉三大彩种的玩法，并可亲自下注带领玩家下注，对下注号码提供专业的分析，以供玩家参考，中奖号码播报，开奖倒计时提醒玩家下注，玩家中奖发送贺彩，语音播报中奖信息，满足玩家虚荣心理，调动其他玩家的下注热情，对于下注额度达到一定金额，进行相应表演，增强彩票玩法的娱乐性与互动性。</div>
               </div>
            </div>
            <div class="section">
               <div class="pic revealOnScroll" data-animation="fadeInUp">
                    <img class="img1" style="position:relative;z-index:2;" src="~assets/images/home/Screen1.png" alt="">
                    <img class="img1" style="position:absolute;left:20%;top:30px;" src="~assets/images/home/screen.png" alt="">
               </div>
               <div class="info revealOnScroll" data-animation="fadeInUp">
                   <h3 class="title">定义博娱新时代</h3>
                   <div class="lv">Live Video</div>
                   <div class="intro">独特的玩家大神排行榜，根椐胜率、连中、中奖金额等进行相应排行，对于上榜玩家有特权免费或收费分享其注单，引领其他玩家跟单购买，对于认证大神分享的注单有权要求跟单者打赏或分享利润分红，玩家可根椐情况随时打赏主播，钻石礼物、现金礼物随意送。彩播后续还将继续完善产品，更多好玩有趣的功能持续更新中。</div>
               </div>
            </div>
            <div class="section">
               <div class="pic revealOnScroll" data-animation="fadeInUp">
                    <img class="img1" style="position:relative;" src="~assets/images/home/sy_sj_2.png" alt="">
                    <img class="img2" style="position:absolute;left:10%;bottom:0px;width:30%;" src="~assets/images/home/sy_sj_3.png" alt="">
               </div>
               <div class="info revealOnScroll" data-animation="fadeInUp">
                   <h3 class="title">全球博娱游戏直播的开创者</h3>
                   <div class="lv">Live Video</div>
                   <div class="intro">专业的培训让主播熟悉三大彩种的玩法，并可亲自下注带领玩家下注，对下注号码提供专业的分析，以供玩家参考，中奖号码播报，开奖倒计时提醒玩家下注，玩家中奖发送贺彩，语音播报中奖信息，满足玩家虚荣心理，调动其他玩家的下注热情，对于下注额度达到一定金额，进行相应表演，增强彩票玩法的娱乐性与互动性。</div>
               </div>
            </div>
            <div class="bg1"></div>
            <div class="bg2"></div>
            <div class="bg3"></div>
            <div class="bg4"></div>
            <div class="bg5"></div>
            <div class="bg6"></div>
        </div>
    </div>
</template>

<script>
import $ from 'jquery'
export default {
    data(){
        return {
            style:{
                height:0,
            }
        }
    },
    mounted(){
        this.style.height = document.documentElement.clientHeight + 'px';
        let tt = document.documentElement.clientHeight;
        let that = this;
        let win_height_padded = tt * 1.1;
        window.addEventListener('scroll',function(){
            var t = document.documentElement.scrollTop || document.body.scrollTop; 
            that.revealOnScroll(tt,t);
        },false)
    },
    beforeDestroy(){
        window.removeEventListener("scroll",this,false);
    },
    methods:{
        revealOnScroll(tt,t){
            $(".revealOnScroll:not(.animated)").each(function () {
                var $this = $(this), 
                    offsetTop = $this.offset().top; 
                if(t + tt > offsetTop) { 
                    if ($this.data('timeout')) { 
                        window.setTimeout(function(){
                             $this.addClass('animated ' + $this.data('animation')); 
                        }, parseInt($this.data('timeout')));
                    } else {
                        $this.addClass('animated ' + $this.data('animation'));
                    } 
                }   
            })
        },
        controldown(){
            let tt = document.documentElement.clientHeight;
            $('html,body').animate({scrollTop:tt},500);
        }
    }
}
</script>

<style lang="scss">
.m-home{
    width:100%;
    background:#1c1c23;
    .m-home-banner{
        width:100%;
        height:auto;
        background:url(~assets/images/mhome/bannerbg.png) no-repeat center;
        background-size:cover;
        position: relative;
        overflow:hidden;
        .m-banner-logo{
            text-align: center;
            padding-top:80px;
            color:#fff;
            background:url(~assets/images/home/light.png) no-repeat 43% top;
            background-size:55%;
            .logo{
                width:20%;
                animation-delay:0.4s;
                -webkit-animation-delay:0.4s;
            }
            .title{
                font-size:18px;
                margin-bottom:5px;
                animation-delay:0.6s;
                -webkit-animation-delay:0.6s;
            }
            .info{
                font-size:14px;
                animation-delay:11s;
                -webkit-animation-delay:1s;
            }
        }
        .m-banner-img{
            display:flex;
            bottom:0;
            left:0;
            width:100%;
            .bannerimg{
                position: absolute;
            }
            .left{
                .banner1{
                    width:60%;
                    bottom:18%;
                    left:-20%;
                    animation-delay:1.8s;
                    -webkit-animation-delay:1.8s;
                }
                .banner2{
                    width:28%;
                    bottom:22%;
                    left:23%;
                    animation-delay:1.4s;
                    -webkit-animation-delay:1.4s;
                }
                .banner3{
                    width:52%;
                    bottom:3%;
                    left:0;
                    animation-delay:1s;
                    -webkit-animation-delay:1s;
                }
            }
            .right{
                .banner1{
                    width:30%;
                    bottom:27%;
                    right:5%;
                    animation-delay:2.2s;
                    -webkit-animation-delay:2.2s;
                }
                .banner2{
                    width:30%;
                    bottom:15%;
                    right:0%;
                    animation-delay:1.8s;
                    -webkit-animation-delay:1.8s;
                }
                .banner3{
                    width:40%;
                    bottom:5%;
                    right:10%;
                    animation-delay:1.4s;
                    -webkit-animation-delay:1.4s;
                }
                .banner4{
                    width:65%;
                    bottom:0;
                    right:-15%;
                    animation-delay:1s;
                    -webkit-animation-delay:1s;
                }
            }
        }
        .control-down{
            width: 51px;
            height: 57px;
            position: absolute;
            top: 89%;
            left: 50%;
            z-index: 999;
            margin: 0 0 0 -25px;
            cursor: pointer;
            text-align: center;
            .down-page {
                width: 51px;
                height: 57px;
                background-position: 0 -83px;
                display: none;
                animation: wave 1.5s both infinite;
                transition: all 1s;
                .icon-darr {
                    margin: 20px 0 0;
                    width: 29px;
                    height: 17px;
                    background-image: url(~assets/images/home/down.png);
                    background-repeat: no-repeat;
                    background-position:center;
                    background-size:100%;
                    display: inline-block
                }

            }
        }
    }
    .m-home-part{
        width:100%;
        position: relative;
        overflow:hidden;
        .section{
            width:100%;
            padding-top:150px;
            padding-bottom:100px;
            position: relative;
            z-index: 2;
            .pic{
                position: relative;
                text-align: center;
                .img1{
                    width:75%;
                }
                .img2{
                    width:22%;
                    position: absolute;
                    bottom:0;
                    left:10%;
                }
                .avatar{
                    position: absolute;
                    top:0;
                }
                .tx1{
                    width:38%;
                    left:10%;
                    top:30px;
                }
                .tx2{
                    width:38%;
                    right:10%;
                    top:55px;
                }
                .tx3{
                    width:22%;
                    left:40%;
                    top:8px;
                }
                .tx4{
                    width:22%;
                    left:38%;
                    top:138px;
                }
            }
            .info{
                width:100%;
                margin-top:50px;
                .title{
                    text-align: center;
                    color:#fff;
                    font-size:17px;
                }
                .lv{
                    color:#677897;
                    font-size:17px;
                    text-align: center;
                    margin:10px 0;
                }
                .intro{
                    width:80%;
                    margin:0 auto;
                    font-size:14px;
                    color:#677897;
                    line-height:25px;
                }
            }
        }
        .bg1{
            width:100%;
            height: 410px;
            background:url(~assets/images/home/homebg.png) no-repeat;
            background-size:cover;
            position: absolute;
            top:0;
            left:0;
        }
        .bg2{
            width:100%;
            height:410px;
            background:url(~assets/images/home/homebgop.png) no-repeat;
            background-size:cover;
            position: absolute;
            top:0;
            left:0;
        }
        .bg3{
            width:100%;
            height:390px;
            background:url(~assets/images/home/bg2.png) no-repeat;
            background-size:100%;
            position: absolute;
            top:550px;
            left:-60%;
        }
        .bg4{
            width:100%;
            height:390px;
            background:url(~assets/images/home/bg2.png) no-repeat;
            background-size:100%;
            position: absolute;
            top:1100px;
            right:-60%;
        }
        .bg5{
            width:100%;
            height:390px;
            background:url(~assets/images/home/bg1.png) no-repeat left;
            background-size:cover;
            position: absolute;
            top:1800px;
            left:-60%;
        }
        .bg6{
            width:100%;
            height:390px;
            background:url(~assets/images/home/bg2.png) no-repeat;
            background-size:100%;
            position: absolute;
            top:2500px;
            right:-60%;
        }
    }
}
@keyframes fadeInLeft {
    0% {
        opacity: 0;
        transform: translate3d(-200%,0,0)
    }

    to {
        opacity: 1;
        transform: none
    }
}

.fadeInLeft {
    animation-name: fadeInLeft
}

@keyframes fadeInRight {
    0% {
        opacity: 0;
        transform: translate3d(200%,0,0)
    }

    to {
        opacity: 1;
        transform: none
    }
}

.fadeInRight {
    animation-name: fadeInRight
}

@keyframes fadeInUp {
    0% {
        opacity: 0;
        transform: translate3d(0,100%,0)
    }

    to {
        opacity: 1;
        transform: none
    }
}

.fadeInUp {
    animation-name: fadeInUp
}

@keyframes wave {
    0%,100% {
        transform: translate3d(0,0,0)
    }

    50% {
        transform: translate3d(0,15px,0)
    }
}
</style>


